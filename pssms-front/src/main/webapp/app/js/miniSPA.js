...
//404 page
var notfound = {};
notfound.partial = "../tmpl/404.html";
notfound.init = function () {
    console.log("init page:notfound");
...
miniSPA.refresh = function (node, scope) {
    ...
    if (node.nodeType != 3) {                            //traverse child nodes, Node.TEXT_NODE == 3
        for (var k = 0; k < node.attributes.length; k++) {
            node.setAttribute(node.attributes[k].name, miniSPA.feedData(node.attributes[k].value, scope));       //replace variables defined in
                                                                                                                 // attributes
        }
...
//replace variables with data in current scope
...
miniSPA.ajaxRequest('../tmpl/404.html', 'GET', '', function (status, partial) {
